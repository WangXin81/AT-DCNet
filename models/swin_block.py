...
class window_attention(nn.Module):
    def __init__(self, dim, dim_head, num_heads=8, qkv_bias=False, dropout=0.0):
        super().__init__()

        self.num_heads = num_heads
        self.inner_dim = dim_head * self.num_heads
...
class cross_window_attention(nn.Module):
    def __init__(self, dim, dim_head, num_heads=8, qkv_bias=False, dropout=0.0):
        ...
        self.num_heads = num_heads
        inner_dim = dim_head * self.num_heads
    ...
    def forward(self, x1, x2, mask=None):
        # x: [B*num_window, num_patches, embed_dim]
        B1, num_patches, total_dim, h = x1.shape[0], x1.shape[1], x1.shape[2], self.num_heads
        B2 = x2.shape[0]
...
class Swin_Block(nn.Module):
    def __init__(self, dim, dim_head, num_heads, Out_res, window_size, dropout=0.0, qkv_bias=False, shift_size=0):
        super().__init__()
        ...
        self.atten = window_attention(dim, dim_head, num_heads, qkv_bias, dropout=dropout)
        self.mlp_norm = nn.LayerNorm(dim)
        self.mlp = MLP(dim, dropout=dropout, mlp_ratio=4)
...
class cross_Swin_Block(nn.Module):
    def __init__(self, dim, dim_head, num_heads, Out_res, window_size, qkv_bias=False, shift_size=0, dropout=0.0):
        super().__init__()
        ...
        self.shift_size = shift_size
        self.atten_norm = nn.LayerNorm(dim)
        self.atten = cross_window_attention(dim, dim_head, num_heads, qkv_bias)
        self.mlp_norm = nn.LayerNorm(dim)
        self.mlp = MLP(dim, dropout=dropout, mlp_ratio=4)
