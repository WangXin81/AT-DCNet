...
class FactorAtt_ConvRelPosEnc(nn.Module):            # 卷积注意模块
    """ Factorized attention with convolutional relative position encoding class. 具有卷积相对位置编码类的因式分解注意力。"""
...
class PositionalEncoding2D(nn.Module):       # 二维位置编码
    def __init__(self, channels):
        ...
        inv_freq = 1. / (10000
                         ** (torch.arange(0, channels, 2).float() / channels))            # 计算正弦函数输入的频率
        self.register_buffer('inv_freq', inv_freq)            # 将频率注册为模块的缓冲区，以便在模型中共享
...
class MultiHeadCrossAttention(MultiHeadAttention):                                # 尺度内交叉交互模块
    ...
    def forward(self, Y, S):
        ...
        cur1 = self.factoratt_crpe(q_g, k_l, v_l, size).permute(0, 2, 1).reshape(Yb, Sc, Yh, Yw)        # M1
        cur2 = self.factoratt_crpe(q_l, k_g, v_g, size).permute(0, 2, 1).reshape(Yb, Sc, Yh, Yw)       # M2
...
class decode(nn.Module):                   # 掩码聚合(MA)模块
    ...
    def forward(self, left, down):
        ...
        left_mask = self.conv_l(left)                                                                                                                                 # mask2
        if down.size()[2:] != left.size()[2:]:
            down_ = F.interpolate(down, size=left.size()[2:], mode='bilinear')
            z1 = F.relu(left_mask * down_, inplace=True)                                                         # f2
        else:
            z1 = F.relu(left_mask * down, inplace=True)

        if down_mask.size()[2:] != left.size()[2:]:
            down_mask = F.interpolate(down_mask, size=left.size()[2:], mode='bilinear')            # mask1

        z2 = F.relu(down_mask * left, inplace=True)             # f1
...
class Up(nn.Module):   # 基于注意机制的空间对齐(SA)模块
...
class Attention_block(nn.Module):     # 计算高尺度的注意力分数
...
class ICIFNet(nn.Module):
    def __init__(self, num_classes=2, drop_rate=0.2, normal_init=True, pretrained=False):
        ...
        self.resnet = resnet18(in_channels= 3, pretrained=pretrained)
        # if pretrained:
        ...
        path = 'E://遥感图像变化检测//论文代码//ICIF-Net-main//pretrained//pvt_v2_b1.pth'
    ...
    def forward(self, imgs1, imgs2, labels=None):
        ...
        final2 = self.upsamplex4(torch.abs(out2_g - out2_g_img2))         # image1和image2的M1
        final1 = self.upsamplex4(torch.abs(out2_l - out2_l_img2))           # M2
        finalx = self.upsamplex4(torch.abs(x_up_3 - x_up_3_img2))          # M3

        map_x = self.final_2(final2)         # 分类器
