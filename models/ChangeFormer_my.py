...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=(stride, stride),
                              padding=((patch_size[0]-stride+1) // 2, (patch_size[1]-stride+1) // 2))
...
class DWConv(nn.Module):  # 深度可分离卷积
    def __init__(self, dim=768):
        ...
        self.dwconv = nn.Conv2d(dim, dim, (3, 3), (1, 1), 1, bias=True, groups=dim)  # 只有逐通道卷积
        # groups参数将卷积操作分组为dim组，每个组内有一个通道，实现深度可分离卷积
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()  # 随机深度,它随机丢弃网络中的一些路径（或者说跳跃连接），以减少过拟合。
...
class MLP(nn.Module):  # MLP    Linear Embedding全连接层
...
class EncoderTransformerV3(nn.Module):
    ...
    def forward_features(self, x):
        ...
        outs = []
        # stage 1
        x1, H1, W1 = self.patch_embed1(x)       # 256->64
...
class ChangeFormerV5(nn.Module):
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        ...
        self.Tenc_x2 = EncoderTransformerV3(img_size=256, patch_size=3, in_chans=input_nc, num_classes=output_nc,
        ...
        # Transformer Decoder
        self.TDec_x2 = DecoderTransformerV3(input_transform='multiple_select', in_index=[0, 1, 2, 3],
                                            align_corners=False,
...
class ChangeFormerV6(nn.Module):
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        ...
        self.attn_drop = 0.1
        self.drop_path_rate = 0.1
        self.Tenc_x2 = EncoderTransformerV3(img_size=256, patch_size=7, in_chans=input_nc, num_classes=output_nc,
                                            embed_dims=self.embed_dims,
