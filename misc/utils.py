...
__all__ = ['cls_accuracy', 'get_scheduler']
...
def get_scheduler(optimizer, args):
    ...
    if args.lr_policy == 'linear':
        ...
        max_step = args.niter + args.niter_decay
        power = 0.9

        def lambda_rule(epoch):
            current_step = epoch + args.epoch_count
        ...
        scheduler = lr_scheduler.StepLR(optimizer, step_size=args.lr_decay_iters, gamma=0.2)
    else:
...
def min_max(tensor):  # 用于计算输入张量 tensor 中每行的最小值和最大值
...
def visulize_features(features, normalize=False):
    ...
    grid = make_grid(features)  # 将一组图像特征（features）组合成一个网格状的图像
...
def seed_torch(seed=2019):
    ...
    np.random.seed(seed)
    torch.manual_seed(seed)  # 设置CPU上PyTorch的随机数生成器的种子
    torch.cuda.manual_seed(seed)


def simplex(t: Tensor, axis=1) -> bool:  # 检查输入的张量是否满足"simplex"条件
    _sum = t.sum(axis).type(torch.float32)
    _ones = torch.ones_like(_sum, dtype=torch.float32)
    return torch.allclose(_sum, _ones)  # 检查一个张量是否表示一个概率分布，其中所有元素都是非负的，并且它们的总和等于1
...
class ChannelMaxPool(MaxPool1d):
    def forward(self, in_input):
        n, c, w, h = in_input.size()
        in_input = in_input.view(n, c, w * h).permute(0, 2, 1)
        pooled = F.max_pool1d(in_input, self.kernel_size, self.stride,
...
class ChannelAvePool(AvgPool1d):
    def forward(self, in_input):
        n, c, w, h = in_input.size()
        in_input = in_input.view(n, c, w * h).permute(0, 2, 1)
        pooled = F.avg_pool1d(in_input, self.kernel_size, self.stride,
                              self.padding)
        _, _, c = pooled.size()
        pooled = pooled.permute(0, 2, 1)
        return pooled.view(n, c, w, h)
...
def cross_entropy(in_input, target, weight=None, reduction='mean', ignore_index=255):
    """
    logSoftmax_with_loss
    :param ignore_index:
    :param reduction:
    :param in_input: torch.Tensor, N*C*H*W
...
def balanced_cross_entropy(in_input, target, weight=None, ignore_index=255):
    """
    类别均衡的交叉熵损失，暂时只支持2类
    TODO: 扩展到多类C>2
    """
    ...
    loss_pos = cross_entropy(in_input, target_pos, weight=weight, reduction='sum', ignore_index=ignore_index)
    loss_neg = cross_entropy(in_input, target_neg, weight=weight, reduction='sum', ignore_index=ignore_index)
...
def save_visuals(visuals, img_dir, name, save_one=True, in_iter='0'):
    ...
    for label, image in visuals.items():
        ...
        for j in range(N):
            ...
            # print(image_numpy)
            img_path = os.path.join(img_dir, in_iter + '_%s_%s.png' % (name_, label))
